...
spider <- read.csv("data/spider.csv", header = TRUE) 

#base approach to loading csv's
spidertv <- read_csv("data/spider.csv") #readr approach to loading csv's
...
spidertv2 <- read_csv("data/spider.csv", col_types = "iDffnffnnnnn")
#readr approach to loading csv's 

#Can also add column names in here too
spidertv3 <- read_csv("data/spider.csv", col_names = c("year", "date", "season", "island", "birddensity", "birdpres", "site", "surveynum", "totalwebs", "time", "length", "webs10m"), skip=1) 
#note that when you use col_names, the first row of data is read in as data, so need to use skip=1 to skip the header row

#If you want to read certain things in as "NA", use na= argument
spidertv4 <- read_csv("data/spider.csv", na = c("na", "NA", ""))

# Reading in excel files is easy too, with readxl package
library(readxl)

#Can read in the second sheet by writing "sheet = 2"
spiderprey <- read_excel("data/Prey_Capture_Final_Do_Not_Touch.xlsx", sheet = 2)

#how does this compare to the csv files we read in earlier? (note - it's a different dataset)
str(spiderprey)
#answer: excel contains factors
